...
overview: "更新 extension_launch_checklist skill：新增「Step 3: 发布前测试」作为上架硬性门禁——运行 tests/ 下全部测试套件（当前含 Linux 兼容性测试），全部 PASS 才允许发布；测试用例落位项目 tests/ 目录并规范写入 .codebuddy/rules/测试用例.mdc；原 Step 3 顺延 Step 4。"
todos:
  - id: validate-skill-spec
    content: 依据 [skill:skill-creator] 校验 SKILL.md 与 .mdc 规范，确认插入方案
    status: pending
  - id: insert-pre-release-step
    content: 在 SKILL.md 插入 Step 3 发布前测试（门禁语义+套件扫描），原 Step 3 顺延 Step 4 追加报告小节
    status: pending
    dependencies:
      - validate-skill-spec
...
## 产品概述
更新 `extension_launch_checklist` skill 与项目测试基础设施：将新增步骤命名为「发布前测试」，作为**发布/上架前的硬性门禁**——检测并运行 `tests/` 目录下的全部测试套件，**所有套件全部通过才允许发布**；任何失败均阻断发布并输出修复清单。首个套件为 Linux 成品兼容性测试（`tests/linux-compat/`），验证已构建成品扩展在 Linux Chromium 中的安装与使用。

## 核心功能
- **发布门禁语义**：Step 3「发布前测试」在发布/上架流程中必执行；检测 `tests/` 下全部套件（目录内存在 `check.sh` 即视为套件），逐套件运行，全部 PASS 放行，任一 FAIL 阻断并给出修复项
- **测试用例独立落位**：测试脚本与断言配置存放于项目 `tests/linux-compat/`（套件目录约定可扩展），随项目版本控制、CI 可复用；skill 仅承载方法论，项目缺失时生成套件模板
...
- **测试用例规范入项目 rules**：新增 `.codebuddy/rules/测试用例.mdc`，固化套件目录约定、文件命名、配置 schema、调用方式、发布门禁语义与平台差异检查约定
- **报告联动**：Step 4 报告追加「发布前测试」小节，记录各套件结果与门禁结论，沿用【事实】/【推测】置信度标注
...
- **SKILL.md Step 3「发布前测试」**：定义发布门禁流程——扫描 `tests/` 下套件、逐套件运行、汇总判定；指引首个套件 `tests/linux-compat/`；若项目缺失套件则生成模板
- **tests/linux-compat/check.sh**：固化断言脚本（项目资产，随版本控制，CI 可复用），阶段 0~4（产物同步/静态预检/加载验证/功能 smoke/平台专项），输出机器可判定 PASS/FAIL 清单与耗时统计
...
└── SKILL.md                      # [MODIFY] 插入 Step 3 发布前测试（门禁语义），原 Step 3 顺延 Step 4，报告追加小节
...
- 新增 **Step 3: 发布前测试**（插入 Step 2 之后）：
  - **门禁语义**：发布/上架前必执行；运行 `tests/` 下全部套件（目录内存在 `check.sh` 即视为套件），全部 PASS 放行，任一 FAIL 阻断发布并输出修复清单
  - 套件执行：对每个套件目录执行 `bash tests/<suite>/check.sh <产物目录>`，汇总退出码与 PASS/FAIL 清单
  - 首个套件指引：`tests/linux-compat/`（Linux 成品兼容性验证），含首次初始化（`sudo apt install -y chromium` + `check.sh --init` + 基线校准）、每次测试流程表（阶段 0~5 及耗时/重要性）、常见坑清单
  - 本机基线：实测值（Ubuntu 24.04.1 LTS / 内核 6.18.35.2-microsoft-standard-WSL2 / 16 核 / 15Gi / git 2.54.0 / chromium 未装）
  - 触发词衔接：在发布/上架相关触发词下与 Step 1 联动必执行
...
- 阶段实现：
  ...
  - 阶段 3：读取配置 smoke.url/smoke.injectFlag，`--dump-dom` 断言注入标志存在，日志无 Uncaught/ERROR；配置缺失时跳过并在输出标注
...
- 正文：发布门禁语义（发布前必须全部通过）、套件目录约定（tests/<suite>/check.sh + 配置）、扩展方式（新增套件即新增目录）、配置 schema、调用方式（`bash tests/linux-compat/check.sh <产物目录>`）、与 extension_launch_checklist skill 的联动、边界（不重建产物/不装 bun/node）、平台差异检查约定（GLIBC ≤ 2.39、大小写敏感、权限位）
