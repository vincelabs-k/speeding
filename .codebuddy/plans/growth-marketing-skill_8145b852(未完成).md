...
overview: 为 Speeding 项目（可跨项目拷贝复用）新建一个「全链路增长/社区营销」skill，面向欧美英文社区高 ARPU 人群，人设为增长负责人（非代码助手）。覆盖定位→渠道→多类型英文内容生成（60/30/10 配比）→发布→数据复盘全链路。长期以盈利为目标，但对外发布内容遵循「盈利提及纪律」：正文以价值叙事为主，付费/盈利只在文末以轻量自然方式提及一次，不频繁强调。
todos:
  - id: init-skill-structure
    content: 用 [skill:skill-creator] 初始化 growth-marketing 骨架，[subagent:code-explorer] 审查现有 skill 风格基准
    status: pending
  - id: write-skill-main
    content: 编写 SKILL.md：增长负责人人设、能力边界声明、四模式工作流、60/30/10 配比表、盈利纪律自检项，全部通用化
    status: pending
    dependencies:
      - init-skill-structure
  - id: write-channels-templates
    content: 编写 channels-handbook.md（欧美渠道+配比）与 copywriting-templates.md（英文模板+文末支持区可选）
    status: pending
    dependencies:
      - write-skill-main
  - id: write-positioning-pitfalls-metrics
    content: 编写 positioning-framework.md（变现路径内敛表述）、launch-pitfalls.md（含正文不提付费纪律）、metrics-review.md
    status: pending
    dependencies:
      - write-skill-main
  - id: write-creative-scripts
    content: 编写 creative-scripts.md：视频脚本/图文帖/视觉创意框架，联动 [skill:多模态内容生成]
    status: pending
    dependencies:
      - write-skill-main
  - id: validate-link-skill
    content: 用 [skill:store-listing-copywriter] 校准禁用词与盈利纪律，[skill:skill-creator] 校验结构，确认联动 utm-sources.md
    status: pending
    dependencies:
...
- **盈利纪律（对外内容）**：正文以价值叙事为主，不把付费/赚钱当卖点；付费/盈利信息只允许在文末以轻量、自然、一次性方式提及（如支持/捐赠/Pro 简介一句话）；内部策略以长期商业价值为锚但表述内敛
...
- **避坑与合规**：社区自我推广规则、禁用绝对化用语、截图真实性、utm 先登记后发布、禁止群发同一篇、正文不提付费
...
- **盈利纪律写入**：SKILL.md 自检清单新增「正文无付费/盈利提及，文末至多一次轻量提及」；模板结构固定结尾「支持区（可选）」
...
数据流：用户提问（推广/引流类）→ 触发 growth-marketing → 按模式执行并读取项目 docs 获取产品信息 → 按内容类型套用 60/30/10 配比输出英文推广方案/软文/脚本（正文不提付费，文末可选轻量提及）→ 发布后按 utm 数据复盘迭代。
...
- 复盘框架以 utm_source 维度驱动（安装→留存→转化漏斗），聚焦高价值渠道加权，指标表述内敛（用"长期可持续"而非"盈利"高频出现）

## 目录结构
```
.codebuddy/skills/growth-marketing/
├── SKILL.md                     # [NEW] 技能主文件。frontmatter 触发条件（推广/引流/软文/launch/growth/marketing 类提问）；正文定义增长负责人人设（含能力边界声明）、四模式工作流、模式 C 内容类型 60/30/10 配比调整表、统一自检清单（utm 登记、禁用词、社区规则、截图真实性、配比自检、正文无付费提及/文末至多一次）。全部通用化，不硬编码产品名。
└── references/
    ...
    ├── copywriting-templates.md # [NEW] 英文软文模板库。Show HN / Reddit 帖子 / IH 帖子 / X 帖子与线程 / 万能软文结构（痛点开头→转折→三点价值→诚实对比→邀请讨论→文末支持区可选）；占位符 {PRODUCT}/{LINK}/{CHANNEL}；每模板标注该载体 60/30/10 配比、内嵌渠道规则提示、固定"结尾支持区（可选）"结构以落实盈利纪律。
    ├── positioning-framework.md # [NEW] 定位与画像框架。高 ARPU 用户画像提炼、差异化卖点挖掘、价值主张句式、诚实对比框架、变现路径设计（免费→信任→口碑/捐赠/Pro）——变现路径表述内敛，以"长期可持续"为价值锚。
    ...
    ├── launch-pitfalls.md       # [NEW] 发布避坑清单。社区自我推广规则、禁用绝对化用语（与 store-listing-copywriter 一致）、截图真实性、utm 先登记后发布、禁止群发同一篇、发布节奏（先集中 1-2 渠道跑通）、正文不提付费/文末至多一次轻量提及。
