---
description: >-
  生成符合品牌规范的浏览器插件图标，采用两层固定布局（徽章底托 + 标准中心图标），输出至 public/icon/，支持交互式参数输入。支持 5 种底托类型和 5 套配色方案，可跨项目复用。
---
...
| `--color` | 否 | 配色方案：`ocean` / `sunset` / `forest` / `midnight` / `slate`。不传则根据 `--function` 自动匹配 |
...
|------|---------------|---------------|------------------------|---------|
| `ocean` | `#0071C5` | `#00C7FD` | `#005A9E` | **通用默认**，工具、效率 |
